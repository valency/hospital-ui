var Prompt = {
  ...
  showBottom : function(){    //创建确定 取消按钮
    ...
    if(that.CancelFun){
        prompt_bottom.innerHTML = "<a class='btn btn-blue mr20' id='ConfirmFun'>确定</a><a class='btn' id='CancelFun'>取消</a>";
        ...
        prompt_bottom.innerHTML = "<a class='btn btn-blue' id='ConfirmFun'>确定</a>";
        that.addHandler(that.$("ConfirmFun"),"click",function(){
            that.hide();
            that.ConfirmFun();
        });
  ...
  show : function(){
    ...
    if((bodyHeight-$("#prompt").height())<100){
    	$("#prompt_body").css({
    ...
    $("#prompt").css({
    	"display": "block",
    	"width": this.width,
    	"top": bodyHeight/2-$("#prompt").height()/2,
    	"left": (this.bodyWidth/2-this.width/2)
    });
    if(this.shade){
